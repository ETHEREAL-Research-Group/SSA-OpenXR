...
/// </summary>
[AddComponentMenu("MRTK/UX/Sync State Visualizer")]
[RequireComponent(typeof(Animator))]
public class SyncStateVisualizer : NetworkBehaviour
{
  ...
  public override void OnNetworkSpawn()
  {
    ...
    CanInteract = temp.Role == "Client";
  }

  protected override void OnOwnershipChanged(ulong previous, ulong current)
  {
    base.OnOwnershipChanged(previous, current);
    CanInteract = NetworkManager.Singleton.LocalClientId == current;
  }

  private bool OwnersValueChanged;
  [ServerRpc(RequireOwnership = false)]
  private void SetState_ServerRpc(float _disabled, float _passiveHover, float _activeHover, float _selctionProgres, float _toggle)
  {
    SetState_ClientRpc(_disabled, _passiveHover, _activeHover, _selctionProgres, _toggle);
  }

  [ClientRpc]
  private void SetState_ClientRpc(float _disabled, float _passiveHover, float _activeHover, float _selctionProgres, float _toggle)
  {
    if (CanInteract) { return; }
    UpdateStateValue("Disabled", _disabled);
    UpdateStateValue("PassiveHover", _passiveHover);
    UpdateStateValue("ActiveHover", _activeHover);
    UpdateStateValue("Select", _selctionProgres);
    UpdateStateValue("Toggle", _toggle);
    OwnersValueChanged = true;
    SyncStateChange.Invoke();
  }
  ...
  UnityEvent SyncStateChange = new UnityEvent();
  /// <summary>
  ...
  protected virtual void Start()
  {
    if (IsHost)
    {
      // interactable.DisableInteractorType(SystemInterfaceType)
    }
    OnValidate();
    ...
    if (interactable != null)
    {
      ...
      unsubscribeActions.Add(Subscribe(SyncStateChange, WakeUp));
    }
  ...
  private void Update()
  {
    ...
    if (!IsSpawned)
    {
      valueChanged = UpdateStateValues();
    }
    else
    {
      if (CanInteract)
      ...
      {
        if (OwnersValueChanged)
        {
          valueChanged = true;
          OwnersValueChanged = false;
        }
        else
          valueChanged = false;
      }
  ...
  protected virtual bool UpdateStateValues()
  {
    if (interactable != null)
    {
      using (StateVisualizerUpdateStateValuesMarker.Auto())
      {
        ...
        if (parameterChanged && IsSpawned)
        {
          SetState_ServerRpc(!interactable.enabled ? 1 : 0, interactable.isHovered ? 1 : 0, interactable.IsActiveHovered ? 1 : 0, interactable.GetSelectionProgress(), interactable.IsToggled ? 1 : 0);
        }
